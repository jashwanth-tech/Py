...
Zero-LaTeX dependency: uses native Pango/Cairo text for 100% portable rendering.
"""
...
class WhatSlopeReallyMeans(Scene):
    def construct(self):
        ...
        # -----------------------------------------------------------
        # Coordinate Plane (Zero-LaTeX: Numbers drawn via native Text)
        # -----------------------------------------------------------
        axes = Axes(
            x_range=[0, 4.5, 1],
            y_range=[0, 6.0, 1],
        ...
        for x_val in range(1, 5):
            axis_numbers.add(
                Text(str(x_val), font_size=15, color=GRAY_B).next_to(
                    axes.c2p(x_val, 0), DOWN, buff=0.15
                )
            )
        for y_val in range(1, 6):
            axis_numbers.add(
                Text(str(y_val), font_size=15, color=GRAY_B).next_to(
                    axes.c2p(0, y_val), LEFT, buff=0.15
                )
            )

        axes_group = VGroup(axes, x_axis_label, y_axis_label, axis_numbers)
